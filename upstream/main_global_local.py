...
class AugmentationModule:
    ...
    def __init__(self, args, size, epoch_samples, log_mixup_exp=True, mixup_ratio=0.4):
        ...
        self.train_transform_local = nn.Sequential(
            MixupBYOLA(ratio=mixup_ratio, log_mixup_exp=log_mixup_exp),
            RandomResizeCrop(virtual_crop_scale=(1.0, 1.5), freq_scale=(0.2, 0.5), time_scale=(0.2, 0.5)),
        )
        ...
        print('Number of heads:', args.number_of_aug)
        self.number_of_aug = args.number_of_aug
    def __call__(self, x):
        if self.norm_status == "byol":
            x = self.pre_norm(x)

        #create the list to store multiple augmentations
        train_transform_list = []
        #global view
        for i in range(2):
            train_transform_list.append(self.train_transform(x))
        #local view
        for i in range(self.number_of_aug-2):
            train_transform_list.append(self.train_transform_local(x))
...
def main(gpu, args):
    ...
    #implementation of multihead as augmentation

    tfms = AugmentationModule(args, (64, 96), args.number_of_aug * len(list_of_files_directory))
...
def train(args, loader, model, optimizer, epoch, gpu, scaler, stats_file):
    ...
    for i, input_tensor in enumerate(loader):
        ...
        input_var = []
        for i in range(args.number_of_aug):
            input_var.append(input_tensor[i].cuda(gpu, non_blocking=True))
        ...
        # record loss
        losses.update(loss, input_var[0].size(0))
...
if __name__== "__main__":
    ...
    args.rank = 0
    args.dist_url = 'tcp://localhost:58462'
    args.world_size = 4
